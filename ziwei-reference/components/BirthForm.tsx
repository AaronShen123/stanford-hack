...
export default function BirthForm({ onSubmit, loading, initialData, onFormSave, hideSubmit }: BirthFormProps) {
  ...
  const errors = {
    year: !form.year ? 'Select a birth year'
      : y < 1900 || y > 2026 ? 'Year must be 1900–2026'
      : '',
    month: !form.month ? 'Select a month' : '',
    day: !form.day ? 'Select a day'
  ...
  const summaryText = showSummary
    ? [
        ...
        form.unknownTime ? 'time unknown' : `${SHICHEN_NAMES[branch]} hour`,
  ...
  return (
    ...
    >
      ...
      {/* ── 出生地点 ── */}
      <div style={{ marginBottom: '16px' }}>
        <label style={{ display: 'block', fontSize: '11px', color: labelClr, marginBottom: '6px', letterSpacing: '0.05em' }}>Birthplace (optional — for true-solar-time correction)</label>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
          <select
            ...
            onBlur={e => { e.target.style.borderColor = inputBorder; }}
          >
            <option value="">Province / Region (CN)</option>
            {PROVINCES.map(p => (
        ...
        <AnimatePresence mode="wait">
          {form.province ? (
            ...
            >
              {form.city || '(select a city)'} · Longitude {form.longitude.toFixed(1)}°E · offset {offsetMin > 0 ? '+' : ''}{offsetMin} min
            </motion.p>
          ) : (
            <motion.p
              key="location-hint"
            ...
            >
              * Optional. Provide a birthplace to auto-correct the hour to true solar time; otherwise the clock time is used.
            </motion.p>
          )}
        </AnimatePresence>
      </div>
      ...
      {/* ── 出生时间 ── */}
      <div style={{ marginBottom: '16px' }}>
        <label style={{ display: 'block', fontSize: '11px', color: labelClr, marginBottom: '6px', letterSpacing: '0.05em' }}>Birth Time (local clock)</label>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '8px' }}>
            ...
            >
              {Array.from({ length: 24 }, (_, i) => i).map(h => (
                <option key={h} value={String(h)}>{h.toString().padStart(2, '0')}h</option>
              ))}
            </select>
            <select
            ...
            >
              {Array.from({ length: 60 }, (_, i) => i).map(min => (
                <option key={min} value={String(min)}>{min.toString().padStart(2, '0')}m</option>
              ))}
            </select>
          </div>
          {/* 真太阳时结果 */}
          <div style={{ textAlign: 'center', padding: '4px 0' }}>
            <span style={{ fontSize: '10px', color: isDark ? 'rgba(170,195,220,0.75)' : 'rgba(140,100,20,0.5)' }}>True solar hour → </span>
            <span style={{ fontSize: '15px', color: goldText, fontWeight: 600, letterSpacing: '0.08em' }}>
              {SHICHEN_NAMES[branch]} hour
            </span>
            {shichenInfo && (
        ...
        <label style={{ display: 'flex', alignItems: 'center', gap: '7px', marginTop: '8px', cursor: 'pointer' }}>
          ...
          <span style={{ fontSize: '10px', color: isDark ? 'rgba(165,185,210,0.7)' : 'rgba(140,100,20,0.45)' }}>
            Birth time unknown — cast using the Zi hour (23:00–01:00)
          </span>
        </label>
      </div>
