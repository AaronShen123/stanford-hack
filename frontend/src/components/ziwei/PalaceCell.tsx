...
import type { Palace, Star } from '../../ziwei/types';
import { tStar, tPalace, tSiHua, STEMS_EN, BRANCHES_EN } from '../../ziwei/i18n';
...
export default function PalaceCell({
  ...
  const ganzhi = `${STEMS_EN[stem]}-${BRANCHES_EN[branch]}`;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1 mb-0.5 pr-8">
        ...
        {isMingGong && (
          <span className="text-[7px] text-amber-500/80 border border-amber-500/30 px-0.5 rounded leading-tight">Life</span>
        )}
        {isShenGong && (
          <span className="text-[7px] text-sky-500/80 border border-sky-500/30 px-0.5 rounded leading-tight">Body</span>
        )}
      </div>
      ...
      <div className="flex flex-col gap-0.5 flex-1">
        {majorStars.length === 0 && (
          <span className="text-[10px] italic" style={{ color: 'var(--t-faint)', opacity: 0.6 }}>Empty</span>
        )}
        {majorStars.map((star) => {
        ...
        <div className="flex flex-wrap gap-x-1">
          {shaStars.map(s => (
            <span key={s.name} className="text-[9px] text-red-500/60 leading-tight">
              {tStar(s.name)}{s.siHua && <SiHuaBadge siHua={s.siHua} />}
            </span>
          ))}
        </div>
      )}
