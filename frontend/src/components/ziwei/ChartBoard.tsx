...
import type { ZiweiChart, Palace, Star } from '../../ziwei/types';
import { BRANCHES, STEMS } from '../../ziwei/constants';
import { BRANCHES_EN, tBureau, tPalace } from '../../ziwei/i18n';
    ...
    <div className="w-full select-none">
      ...
      >
        ...
        <h2 className="text-sm tracking-[0.25em] font-medium" style={{ color: 'var(--t-gold)' }}>
          {chart.birthInfo.name ? `${chart.birthInfo.name} · ` : ''}Zi Wei Dou Shu Natal Chart
        </h2>
      </motion.div>
      ...
      >
        ...
        >
          ...
          <div className="text-center space-y-1">
            <div className="text-[9px] tracking-[0.3em] font-medium" style={{ color: 'var(--t-gold)' }}>ZI WEI</div>
            <div className="text-[10px] space-y-0.5" style={{ color: 'var(--t-faint)' }}>
              <div>Life <span style={{ color: 'var(--t-gold)', opacity: 0.7 }}>{BRANCHES_EN[chart.mingGongBranch]}</span></div>
              <div>Body <span className="text-sky-500/70">{BRANCHES_EN[chart.shenGongBranch]}</span></div>
              <div className="text-[9px]" style={{ color: 'var(--t-gold)', opacity: 0.75 }}>{tBureau(chart.wuxingJuName)}</div>
            </div>
          </div>
          ...
          {chart.currentDaXianIndex >= 0 && (() => {
            ...
            return (
              <div className="border border-purple-500/30 rounded-lg px-3 py-1.5 text-center"
                ...
                <div className="text-[8px] text-purple-500/80 mb-0.5 tracking-wider">CURRENT DECADE</div>
                <div className="text-[12px] text-purple-400 font-medium tabular-nums">Age {dx.startAge}–{dx.endAge}</div>
                <div className="text-[9px] text-purple-500/60">{tPalace(dx.palaceName)}</div>
              </div>
            );
          })()}
          ...
          <div className="text-[8px] text-center leading-relaxed font-mono" style={{ color: 'var(--t-faint)', opacity: 0.75 }}>
            ...
            M{chart.lunarInfo.lunarMonth} · D{chart.lunarInfo.lunarDay}
          </div>
        </motion.div>
      ...
      >
        {[
          { h: 'Lu · Fortune', c: 'text-emerald-500 border-emerald-500/30' },
          { h: 'Quan · Power', c: 'text-blue-500 border-blue-500/30' },
          { h: 'Ke · Fame', c: 'text-yellow-500 border-yellow-500/30' },
          { h: 'Ji · Affliction', c: 'text-red-500 border-red-500/30' },
        ].map(({ h, c }) => (
        ...
        <span className="px-1.5 py-0.5 rounded-full" style={{ color: 'var(--t-faint)', border: '1px solid var(--t-border)' }}>
          Tap a palace for its trine alignment
        </span>
      </motion.div>
    </div>
  );
